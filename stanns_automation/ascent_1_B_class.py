...
driver.get('https://ascent.tatotechnologies.com/login')
...
# form elements
def test_login():
    username.send_keys('admin_ascentinfo@tatotechnologies.com')
    # time.sleep(5)
    password.send_keys('Ascentinfo@2025')
    # time.sleep(5)
    submit.click()

def test_open_admission():
    time.sleep(3)
    driver.get('https://ascent.tatotechnologies.com/student-admission')
    time.sleep(5)
...
def select_class():
    ...
    select_first_class = driver.find_element(By.XPATH,
                                             "//div[@class='nice-select primary_select form-control open']//li[@class='option'][normalize-space()='1 Class']")
...
wb = load_workbook('C:/Users/veera/Downloads/stanns_1_B_students.xlsx')  # Change to your file name
sheet = wb.active
...
def test_upload_student_bulk_data(admission_number,first_name,date,student_mail,parent_mail,date_of_birth):
    ...
    select_section('C') # selects A section
...
def test_add_data_to_portal():
    ...
    for i, row in enumerate(sheet.iter_rows(min_row=2, values_only=True), start=2):
            ...
            date_obj1 = datetime.strptime(str(row[5]).split()[0], '%d-%m-%Y')
            ...
            print('the last uploaded name is',first_name)
